...
uint8_t bmi160Detect(const extDevice_t *dev)
{
    ...
    // Force SPI mode: toggle CS low then high to wake GY-BMI160 out of I2C mode
    IO_t csPin = dev->busType_u.spi.csnPin; // <--- FIXED: It is already an IO_t object!
    if (csPin) {
        IOWrite(csPin, true);       // high
        delayMicroseconds(10);
        IOWrite(csPin, false);      // low
        delayMicroseconds(10);
        IOWrite(csPin, true);       // high

        // Give the BMI160 SPI interface 10ms to stabilize
